...
extern int bpf_xdp_store_rx_csum(struct xdp_md *ctx) __ksym;
...
{
	/* Vouch for the L4 checksum so the skb built on the cpumap redirect
	 * path is marked CHECKSUM_UNNECESSARY instead of validated in software.
	 */
	if (store_csum)
		bpf_xdp_store_rx_csum(ctx);
